...
class Event():
    ...
    def __init__(self, filename, CH1=None, CH2=None, CH3=None, CH4=None, I_DC=None, Q_DC=None, label='Event', sigma_tresh=0., sigma_n_bin=200, force_sigma_estimation=False):
        
        self.label = label
        self.time = {'label': 'TIME', 'data': []}
        self.channels = {}
        self.filename = Path(filename)
        self.sigma_tresh = sigma_tresh
        self.I_DC = I_DC
        self.Q_DC = Q_DC
        self.sigma_n_bin = sigma_n_bin
        ...
        try:
            with open(self.filename.parent / (self.filename.stem+'_sigma.pkl'), 'rb') as fp:
                self.sigma_bin_heights, self.sigma_bin_position, self.sigma_fit_results = pickle.load(fp)
            print("Found sigma estimation file!")
        except FileNotFoundError:
            print("Sigma estimation file does not exist. Estimating...")
            self.sigma_bin_heights, self.sigma_bin_position, self.sigma_fit_results = self.compute_errorbars()
            with open(self.filename.parent / (self.filename.stem+'_sigma.pkl'), 'wb') as fp:
                pickle.dump([self.sigma_bin_heights, self.sigma_bin_position, self.sigma_fit_results], fp)

        if force_sigma_estimation:
    ...
    def compute_errorbars(self):
        bin_heights, bin_position = np.histogram(self.A, self.sigma_n_bin)
        ...
        params.add('sigma', value=1.0e-1, min=0.0, max=5.0)
        
        keep = bin_heights >= self.sigma_tresh*bin_max_height
    ...
    def fancyPlotFit(self, save_figure=True, show_figure=False, title=None):
        ...
        # HISTOGRAM PLOT
        plt.stairs(self.sigma_bin_heights, self.sigma_bin_position, orientation='horizontal', fill=False, color='black')
        
        def gaussian(x, params):
            return params['A']*np.exp(- 0.5 * (x-params['mu'])**2.0/params['sigma']**2.0)
            
        x_data = np.linspace(self.sigma_fit_results.params['mu']-5.0*self.sigma_fit_results.params['sigma'], self.sigma_fit_results.params['mu']+5.0*self.sigma_fit_results.params['sigma'], num=500)
        ax_hist.plot(gaussian(x_data, self.sigma_fit_results.params), x_data, color='blue')
