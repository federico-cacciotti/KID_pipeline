    ...
    def __init__(self, filename, label=None, mode='real_imag'):
        ...
        if self.mode == 'real_imag':
            [self.freqs, self.ReS11, self.ImS11, self.ReS21, self.ImS21, 
             self.ReS12, self.ImS12, self.ReS22, self.ImS22] = np.loadtxt(fname=filename, dtype=float, skiprows=23, unpack=True)
        
        if self.mode == 'log_mag_phase':
            [self.freqs, self.S11DB, self.S11A, self.S21DB, self.S21A, 
             self.S12DB, self.S12A, self.S22DB, self.S22A] = np.loadtxt(fname=filename, dtype=float, skiprows=23, unpack=True)
